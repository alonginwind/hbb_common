...
impl WsFramedStream {
    ...
    async fn connect(
        ...
        Err(Error::new(ErrorKind::NotConnected, "all IPs failed").into())
    }
    ...
    async fn try_connect(
        ...
        // Single timeout covers TCP + TLS handshake (matches pre-split behavior).
        let result = timeout(
            Duration::from_millis(ms_timeout),
            async {
                let tcp = TcpStream::connect(addr).await?;
                client_async_tls_with_config(request, tcp, ws_config, connector).await
            },
        )
        .await;
